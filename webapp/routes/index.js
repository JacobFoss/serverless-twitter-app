...
router.get('/', function(req, res, next) {
  ...
  if(!process.env.DB_COLLECTION) {
    process.env.DB_COLLECTION = "twittertag";
  }
  ...
  // Query docs for photos with URL in reverse time order
    client.queryDocuments(collectionUrl, 'SELECT TOP 50 * FROM c WHERE c.url != null AND c.description != null ORDER BY c.time DESC', { enableCrossPartitionQuery: true })
  .toArray((err, results) => {
    if(err) {
      sendError(res, err);
      return;
